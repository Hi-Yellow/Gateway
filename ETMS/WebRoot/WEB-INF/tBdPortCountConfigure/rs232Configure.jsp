<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<html>
<head>
	<%@ include file="/common/common.jspf"%>
	<title>修改</title>
	<base href="${basePath }"/>
	<link rel="stylesheet" href="css/mycss.css" type="text/css"></link>
    
</head>
<body class="mybody">
	<form id="ff" class="myfm" method="post">
		<h2>RS232配置</h2><hr>
	   	<table>
		<tr class="myfitem3">
			<td><label for="terminalCode">终端ID:</label></td> 
			<td><input type="text" id='terminalCode' name="terminalCode" readonly="readonly"/></td>
		</tr>
		<tr class="myfitem3">
			<td><label for="portID">串口号:</label></td>
			<td><input id='portID' name='portID' value="1"/></td>
		</tr>
		<tr class="myfitem3">           
			<td><label for="portName">串口名:</label></td> 
			<td><input type="text" id="portName" name="portName"></input></td>
		</tr>
		<tr class="myfitem3">           
			<td><label for="baudRate">波特率:</label></td> 
			<td><input type="text" id="baudRate" name="baudRate"></input></td>
		</tr>
		<tr class="myfitem3">           
			<td><label for="stopBit">停止位:</label></td> 
			<td><input type="text" id="stopBit" name="stopBit"></input></td>
		</tr>
		<tr class="myfitem3">           
			<td><label for="checkBit">校验位:</label></td> 
			<td><input type="text" id="checkBit" name="checkBit"></input></td>
		</tr>
		<tr class="myfitem3">           
			<td><label for="dataBit">数据位:</label></td> 
			<td><input type="text" id="dataBit" name="dataBit"></input></td>
		</tr>
		</table>
		<tr class="myfitem3">
			<td><a id="btn" class="easyui-linkbutton" 
				data-options="iconCls:'icon-ok'">提交</a></td>  
			<td><a id="btn2" class="easyui-linkbutton" 
				data-options="iconCls:'icon-cancel'" 
				onclick="javascript:parent.$('#win').window('close');" style="float: right">关闭</a></td>  
		</tr>
	</form>

	<script type="text/javascript">
		$(function() {
			var row = parent.$('#dg').datagrid("getSelected");
			row.tBdPortCountConfigurePrivilegeStart = format(row.tBdPortCountConfigurePrivilegeStart);
			$('#terminalCode').val(row.terminalCode);
			$('#portID').combobox({    
			    url:'tBdRS232/selectByCode.action?terminalCode='+row.terminalCode,
			    required : true,
			    editable : false,
				missingMessage : '请选择串口号！',
			    width: 200,	    
			    panelHeight: 'auto',    
			    valueField:'portID',    
			    textField:'portID',
			    onSelect: function(record){
			    	$('#portName').val(record.portName);
			    	$('#baudRate').val(record.baudRate);
			    	$('#checkBit').val(record.checkBit);
			    	$('#stopBit').val(record.stopBit);
			    	$('#dataBit').val(record.dataBit);
			    } 
			});	
			if(row.rs232Count==1){
				$('#portID').combobox('readonly');
			}
			//$('#ff').form('load',row);	
		});
		
		$("#btn").click(function() {
				$("#ff").form("enableValidation");
				if ($("#ff").form("validate")) {
					$('#ff').form('submit', {
						url : 'tBdRS232/update.action',
						onSubmit : function() {
							return true;
						},
						success : function(count) {
							//可以定义为对应消息框
							if(count>0){
								alert("提交成功！");	
							}else if(count==0){
								alert("提交失败！");
							}
							parent.$("#win").window("close");
							parent.$("#dg").datagrid("reload");
						}
					});
				}
			});
	</script>
</body>
</html>
